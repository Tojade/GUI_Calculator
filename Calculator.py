...
def get_variables(num):
    global i 
    display.insert(i,num) #Keeps track of the position of the value that will be inserted
    i+=1

#Parsing the entry box number so it can be edited and interacted
def calculate():
...
def get_operation(operator):
    global i #Using the global variable 
    length = len(operator) #Calculate the length of the operator 
    display.insert(i,operator) #Insert the operator 
    i+=length #The operator is not equal to 1, it will pass in the length
 
#Clear all function using the AC button
def clear_all():
    display.delete(0,END)
 
def undo():
    entire_string = display.get()
    if len(entire_string): #Deleting a signle element of a string by checking the length
        new_string = entire_string[:-1] #The list will get sliced
        clear_all() #Clears the entire string
        display.insert(0,new_string) #Inserts new string which is the old string minus one element
    else:
        clear_all()
        display.insert(0,"Error")
